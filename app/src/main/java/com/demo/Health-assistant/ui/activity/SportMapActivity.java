...
import com.demo.motion.commmon.utils.CountTimerUtil;
import com.demo.motion.commmon.utils.DateUtils;
import com.demo.motion.commmon.utils.LogUtils;
...
import com.demo.motion.db.DataManager;
import com.demo.motion.db.RealmHelper;
import com.demo.motion.sport_motion.MotionUtils;
import com.demo.motion.sport_motion.PathSmoothTool;
...
//        mService = null;
    ...
    private void startLocation() {
        if (mLocationClient == null) {
            ...
            mLocationOption.setGpsFirst(false);//设置是否gps优先，只在高精度模式下有效。默认关闭
            mLocationOption.setHttpTimeOut(30000);//设置网络请求超时时间。默认为30秒。在仅设备模式下无效
            mLocationOption.setInterval(interval);//设置定位间隔。默认为2秒
            mLocationOption.setNeedAddress(false);//设置是否返回逆地理地址信息。默认是true
            mLocationOption.setOnceLocation(false);//设置是否单次定位。默认是false
            mLocationOption.setOnceLocationLatest(false);//设置是否等待wifi刷新，默认为false.如果设置为true,会自动变为单次定位，持续定位时不要使用
            AMapLocationClientOption.setLocationProtocol(AMapLocationClientOption.AMapLocationProtocol.HTTP);//可选， 设置网络请求的协议。可选HTTP或者HTTPS。默认为HTTP
            mLocationOption.setSensorEnable(false);//设置是否使用传感器。默认是false
            mLocationOption.setWifiScan(true); //设置是否开启wifi扫描。默认为true，如果设置为false会同时停止主动刷新，停止以后完全依赖于系统刷新，定位位置可能存在误差
            mLocationOption.setLocationCacheEnable(true); //设置是否使用缓存定位，默认为true
            mLocationOption.setGeoLanguage(AMapLocationClientOption.GeoLanguage.ZH);//设置逆地理信息的语言，默认值为默认语言（根据所在地区选择语言）
