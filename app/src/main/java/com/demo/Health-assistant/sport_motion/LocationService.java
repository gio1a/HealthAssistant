package com.demo.motion.sport_motion;
...
import com.demo.motion.sport_motion.servicecode.RecordService;
import com.demo.motion.sport_motion.servicecode.impl.RecordServiceImpl;
...
//    private List<LatLng> mSportLatLngs = new LinkedList<>();
    ...
    private void initLocationOption() {
        ...
        mOption.setGpsFirst(true);//设置是否gps优先，只在高精度模式下有效。默认关闭
        mOption.setHttpTimeOut(30000);//设置网络请求超时时间。默认为30秒。在仅设备模式下无效
        mOption.setInterval(4000);//设置定位间隔。默认为2秒
        ...
        mOption.setSensorEnable(false);//设置是否使用传感器。默认是false
        mOption.setWifiScan(true); //设置是否开启wifi扫描。默认为true，如果设置为false会同时停止主动刷新，停止以后完全依赖于系统刷新，定位位置可能存在误差
        mOption.setLocationCacheEnable(false); //设置是否使用缓存定位，默认为true
        mOption.setGeoLanguage(AMapLocationClientOption.GeoLanguage.DEFAULT);//设置逆地理信息的语言，默认值为默认语言（根据所在地区选择语言）
